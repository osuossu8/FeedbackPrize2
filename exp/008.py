...
class FeedBackModel(nn.Module):
    def __init__(self, model_name):
        ...
        self.head = AttentionHead(self.config.hidden_size * 2,self.config.hidden_size * 2,1)
...
#         sequence_output = self.pooler(all_hidden_states)
        ...
        x = torch.cat([transformer_out[1][idx] for idx in [0, -1]], dim=2)

        sequence_output = self.head(x) # bs, 2048
