...
skf = GroupKFold(n_splits=CFG.n_splits) # , shuffle=True, random_state=CFG.seed)
train['fold'] = -1
train['label'] = train['discourse_effectiveness'].map({'Ineffective':0, 'Adequate':1, 'Effective':2})
for i, (_, val_) in enumerate(skf.split(train, groups=train['essay_id'])):
    train.loc[val_, 'fold'] = int(i)
train.to_csv('input/train_fold_group.csv', index=False)

train = pd.read_csv('input/train_fold_group.csv')
